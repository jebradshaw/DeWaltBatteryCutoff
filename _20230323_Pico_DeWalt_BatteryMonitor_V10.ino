...
void buzzerUp(void){
  analogWriteFreq(1000);     // set freq output to 2KHz   
  ...
  analogWriteFreq(800);     // set freq output to 2KHz   
  analogWrite(BUZZER_PIN, 512);     // turn on PWM 
  digitalWrite(led, !digitalRead(led));
  delay(200); 
  analogWrite(BUZZER_PIN, 0);     // turn off PWM 
  ...
  analogWriteFreq(600);     // set freq output to 2KHz   
  analogWrite(BUZZER_PIN, 512);     // turn on PWM 
  digitalWrite(led, !digitalRead(led));
  delay(200); 
  analogWrite(BUZZER_PIN, 0);     // turn off PWM 
...
//    digitalWrite(led, !digitalRead(led));
//    delay(100);
//    analogWriteFreq(2000 + (i*1000));     // set freq output to 2KHz
//  }  
...
//    EEPROM.commit();
  ...
  Serial.begin(115200);
  Serial.printf("PICO BATTERY MONITOR V1.0 - 20230323 J.Bradshaw\r\n");
      ...
      if(millis() - critCntLastTime > 1000){
        ...
        if(criticalCounter > CRIT_TIMOUT_SEC){ // seconds until timeout and shutoff
          ...
          while(exitTrap){            
            ...
            Serial.printf("PICOBATMON,T:%d,dVdT:%.3f,Bv:%.2f,Bcrit:%d,Halarm:%.2f,Lalarm:%.2f\r\n", millis(), dVdT, battVolts, buzzerCrit, highAlarmVolts, lowAlarmVolts);
          }
          digitalWrite(D2, HIGH); // turn on the P-Channel Power FET (Pinch off the voltage source to the load)
          buzzerUp();
          buzzerCrit=0;
          buzzerState=0;                  
...
// check is a serial character was received
  if(Serial.available()>0){
    ...
    if(Serial.peek() == '?'){ // ? display commands
        ...
        Serial.printf("eepromw S1 F1 F2\\r - write to EEPROM, S1 is string description, F1 high alarm volts, F2 low alarm volts\r\n");
